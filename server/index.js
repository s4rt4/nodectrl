...
function getDepsInfo(dir, cb) {
  ...
  exec(outdatedCmd, { cwd: dir, timeout: 30000, shell: true }, (_e, stdout) => {
    ...
    if (pm === 'pnpm') {
      ...
      // yarn outdated --json: each line is a JSON object, data row has type:"table"
      (stdout || '').split('\n').forEach(line => {
        try {
          const obj = JSON.parse(line);
          if (obj.type === 'table' && obj.data && obj.data.body) {
            obj.data.body.forEach(row => {
      ...
      // bun outdated: plain text table — parse lines like "package  current  latest"
      (stdout || '').split('\n').forEach(line => {
    ...
    exec(auditCmd, { cwd: dir, timeout: 30000, shell: true }, (_e2, stdout2) => {
      ...
      // pnpm audit JSON has same structure as npm
      const meta = (audit.metadata || {}).vulnerabilities || {};
      cb(null, { deps: Object.values(deps), auditSummary: meta, packageManager: pm });
    });
  });
}
